class RecintosZoo {
    ...
    analisaRecintos(tipoAnimal, quantidade) {
        ...
        this.recintos.forEach(recinto => {
            ...
            let espacoOcupado = recinto.animais.reduce((espaco, a) => espaco + a.quantidade * this.animalTamanho(a.especie), 0);
            let simCarnivoros = recinto.animais.some(a => this.animaisValidos.find(av => av.especie.toUpperCase() === a.especie.toUpperCase()).carnivoro);
            let naoCarnivoros = recinto.animais.some(a => !this.animaisValidos.find(av => av.especie.toUpperCase() === a.especie.toUpperCase()).carnivoro);
    
            if (animal.especie === 'HIPOPOTAMO' && recinto.bioma !== 'savana e rio' && recinto.animais.length > 0) {
                return;
            }
    
            if (animal.carnivoro && naoCarnivoros) {
                return;
            }
    
            if (!animal.carnivoro && simCarnivoros) {
                return;
            }
        ...
        recintosViaveis.sort((a, b) => {
            const A = parseInt(a.match(/Recinto (\d+)/)[1]);
            const B = parseInt(b.match(/Recinto (\d+)/)[1]);
            return A - B;
        });
